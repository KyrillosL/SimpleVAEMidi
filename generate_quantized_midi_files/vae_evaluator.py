...
class Vae:
    def __init__(self):
        ...
        path_midi_file_to_initialize_model = "/Users/Cyril_Musique/Documents/Cours/M2/MuGen/datasets/quantized_rythm_dataset_v2_temperature/100/generated_2.mid"
                ...
                if num_files < self.dataset_size:
                    if file != ".DS_Store":
                        ...
                        if len(midi_data.instruments) > 0:
                            data = midi_data.get_piano_roll(fs=self.res)[36:51, :]
    ...
    def generate(self, data):
        #data = np.reshape(data,[15,32])
       #print(data)

        #data2 = np.reshape(data, [16, 16]).astype(dtype=bool)
        #data1 = data[0]
        print("DATA \n",data)
        #encoded = self.encoder.predict(data[0:1])
        encoded = self.encoder.predict(data[0:1])
        print("ENCODED :\n ", encoded)
        ...
        #final = final #self.number_of_data_to_extract, self.range_of_notes_to_extract

        final = np.reshape(final, [15, 32]).astype(dtype=bool)
